...
# Manual overrides (issue #433): scene names the *curated* community layer
# gets wrong — the last word on ids where the shipped client's own name is
...
def render() -> str:
    ...
    scenes = merge_names(
        ...
        # Issue #433: the final word, above even the curated community
        # layer. See `SCENE_NAME_MANUAL_OVERRIDES` for why a handful of ids
        # need the client's own name back.
        _keyed(SCENE_NAME_MANUAL_OVERRIDES),
    )
