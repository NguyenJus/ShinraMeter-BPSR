...
/// Where the meter is in the fight lifecycle as of a given `now_ms`: the
/// stored [`FightLifecycle`] with the *derived* idle-timeout end folded in
/// — see [`crate::encounter::Meter::fight_state`].
...
pub enum FightEndCause {
    ...
    /// at a player death (issue #154), or at least one known member down at
    /// the moment the boss's HP bar rolled back to full (issue #461).
    Wipe,
...
impl FightLifecycle {
    ...
    /// (leaving an instance, a server change, a battle rez, `Meter::reset`)
    /// drop the hold unconditionally and are reached far more often with no
    /// hold than with one.
