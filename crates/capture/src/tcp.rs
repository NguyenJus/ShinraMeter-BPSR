...
impl TcpReassembler {
    ...
    pub fn push(&mut self, seq: u32, payload: &[u8]) {
        ...
        if after == before && (!self.cache.is_empty() || re_anchored) {
            ...
            self.stall_pushes = 0;
            // Forward progress with nothing left cached: the stream is
            // fully caught up, not just past the one gap that last
            // tripped. Only this — not merely surviving to the next
            // push — earns back the default threshold.
            if self.cache.is_empty() {
