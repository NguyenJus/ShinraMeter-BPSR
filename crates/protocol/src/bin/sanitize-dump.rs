...
//! all is dropped from the output entirely (see [`sanitize::is_modeled`]).
//! That drop is the safety property: nothing exits this tool that isn't
//! explicitly accounted for below. This is deliberately not a surgical
//! byte-patch that preserves unknown fields — that would leak the 36-char
//! session GUIDs riding `EnterScene`'s unmodeled fields 1.7/1.8 and opcode
//! `0x04`'s field 1.2.
//!
//! Player-identifying uids are consistently remapped (see [`sanitize::Remap`])
//! and names are replaced with stable `PlayerNNNNN` placeholders. Two uid
//! shapes exist in the wire format and must not be confused: most uids are
//! *packed* `uuid = (uid << 16) | kind_bits`, but `CharSerialize.char_id` and
//! `CharBaseInfo.char_id` are *bare* uids — applying the packed-uuid rule to
//! them silently corrupts the local player's class/ability_score while
//! leaving every damage number byte-identical (this bug bit an earlier
//! attempt at this tool; the mandatory self-check below is what would have
//! caught it).
//!
...
/// Decode -> rewrite identifying values -> re-encode through the partial
/// `pb.rs` schema. See this file's module doc comment for the safety
...
mod sanitize {
    ...
    /// (starting at 100_000, clear of any real uid range) in first-seen
    /// order so the same player gets the same placeholder everywhere.
    pub struct Remap {
    ...
    impl Remap {
        ...
        /// Remaps a *packed* `uuid = (uid << 16) | kind_bits`, preserving
        /// the low 16 bits (which encode entity kind, not identity).
    ...
    /// Attr ids anything downstream of `decode_notify` actually reads —
    ...
    /// Returns the sanitized, re-encoded payload for one record, or `None`
    /// for an opcode we do not model (dropped from the output entirely) or
    /// a payload that fails to decode against the modeled schema.
        ...
        match method_id {
            ...
            opcode::SYNC_CONTAINER_DATA => {
                ...
                if let Some(v) = &mut m.v_data {
                    ...
                    // `decode::on_sync_container_data`) — remapping it with
                    // the uuid rule silently misattributes the local player
                    // (damage numbers stay byte-identical, only class/
                    // ability_score go missing).
    ...
    mod tests {
        ...
        fn bare_uid_remap_is_stable_and_unbounded_by_the_16_bit_low_half() {
            ...
            // arbitrary-looking uid must be able to land anywhere, and must
            // not silently reuse the input's own low 16 bits the way a
            // packed uuid's remap would.
            ...
            assert_ne!(
                a & 0xFFFF,
                uid & 0xFFFF,
                "a bare uid's low 16 bits must not be pinned through the remap the way a \
                 packed uuid's are — that pinning is `uuid()`-specific structure that does \
                 not apply here"
            );
        }
        ...
        fn packed_uuid_and_bare_uid_rules_diverge_on_the_same_char_id() {
            ...
            let mut r = Remap::new();
            // Same `Remap`, so the two rules would collide here if (and only
            // if) they consult the same underlying uid-table slot for
            // `char_id` — which they must not: the bare rule keys on
            // `char_id` itself (459392), the uuid rule keys on `char_id >>
            // 16` (7), a different player entirely.
        ...
        fn scrub_attrs_drops_unlisted_attr_ids_and_keeps_the_whitelist() {
            ...
            let mut ac = pb::AttrCollection {
                ...
                attrs: vec![
                    ...
                    pb::Attr {
                        id: bpsr_protocol::attrs::attr_id::HP,
                        raw_data: vec![1, 2, 3],
                    },
                    pb::Attr {
            ...
            let name_attr = ac
                .attrs
                .iter()
                .find(|a| a.id == bpsr_protocol::attrs::attr_id::NAME)
                .expect("NAME attr must survive");
            // raw_data is a varint length prefix + UTF-8 bytes; the encoded
            // name must not contain the original real name.
