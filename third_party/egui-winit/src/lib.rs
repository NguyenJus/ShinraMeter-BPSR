...
// ----------------------------------------------------------------------------
// ShinraMeter-BPSR local fork (issue #89) — the *only* deviation from upstream
// egui-winit 0.36.1. Everything below in this block, plus the four lines that
// consult `NO_REDIRECTION_BITMAP` inside `create_winit_window_attributes`, is
// the whole diff. See `third_party/egui-winit/Cargo.toml` for the vendoring
// rationale and the `[patch.crates-io]` wiring.

/// Whether the next window created by [`create_winit_window_attributes`] should
/// carry `WS_EX_NOREDIRECTIONBITMAP` (Windows only; ignored everywhere else).
///
/// A process-global atomic rather than a field on [`ViewportBuilder`] because
/// this fork deliberately does not touch egui's public viewport API: keeping
/// the diff to one file and zero upstream types is what makes the next egui
/// version bump a re-copy rather than a merge.
static NO_REDIRECTION_BITMAP: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

/// Opt into `WS_EX_NOREDIRECTIONBITMAP` for windows created after this call.
///
/// # Why this fork exists (issue #89)
///
/// A transparent, undecorated overlay gets an opaque GDI *redirection surface*
/// allocated by DWM at `CreateWindowEx` time, sized to the window and
/// initialized white. wgpu presents with `alpha_mode: PreMultiplied`, so DWM
/// composites the swapchain over that white surface instead of over the
/// desktop — the panel fill lands on a flat gray (#454548) at startup. Only
/// area the window is later *grown* into has no redirection surface behind it,
/// which is why users reported that resizing "fixes" the overlay in strips.
///
/// `WS_EX_NOREDIRECTIONBITMAP` suppresses that surface, but Windows honors the
/// flag **only** at `CreateWindowEx`: a post-hoc
/// `SetWindowLongPtrW(GWL_EXSTYLE)` is silently ignored (measured). Since winit
/// owns the `CreateWindowEx` call and egui-winit owns the
/// [`winit::window::WindowAttributes`] handed to it, there is no seam in
/// unmodified eframe where an application can inject the flag — hence the fork.
///
/// # Invariants the caller must uphold
///
/// DXGI refuses `CreateSwapChainForHwnd` on a window with this flag, so the
/// caller must guarantee the renderer presents through `DirectComposition`
/// (wgpu's `Dx12SwapchainKind::DxgiFromVisual`) before enabling it. Vulkan is
/// assumed unable to present to such a window at all. `crates/app/src/main.rs`
/// therefore only calls this after probing for a DX12 adapter, and pins
/// `Backends::DX12` in the same breath. Enabling it without that pairing yields
/// a window that never presents anything.
///
/// Must be called before `eframe::run_native` (i.e. before any window is
/// created); flipping it later affects only subsequently created viewports.
///
/// # Dropping this fork
///
/// Upstream egui would need a way to set Windows-specific creation-time window
/// flags — most naturally a `no_redirection_bitmap: Option<bool>` on
/// [`ViewportBuilder`] alongside the existing `drag_and_drop` / `taskbar`
/// Windows fields, plumbed into the `#[cfg(target_os = "windows")]` block of
/// [`create_winit_window_attributes`]. The day egui ships that, delete
/// `third_party/egui-winit/`, drop the `[patch.crates-io]` entry, and set the
/// builder field instead.
pub fn set_no_redirection_bitmap(enabled: bool) {
    NO_REDIRECTION_BITMAP.store(enabled, std::sync::atomic::Ordering::Relaxed);
}
...
pub fn create_winit_window_attributes(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // ShinraMeter-BPSR fork (issue #89): the only behavioural change in this
        // vendored copy. `WS_EX_NOREDIRECTIONBITMAP` is honored solely at
        // `CreateWindowEx`, so this is the last point at which it can be
        // requested. Opt-in — see `set_no_redirection_bitmap` above for the
        // DirectComposition pairing the caller must guarantee.
        if NO_REDIRECTION_BITMAP.load(std::sync::atomic::Ordering::Relaxed) {
